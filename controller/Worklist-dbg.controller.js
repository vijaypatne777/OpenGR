sap.ui.define([
...
], function (BaseController, JSONModel, formatter, Filter, FilterOperator, tn, Label, MT, Spreadsheet, Fbar, Fgitem) {
	"use strict";
	...
	return BaseController.extend("OpenGR.OpenGR.controller.Worklist", {
		...
		onInit: function () {
			...
			this._coModel = this.getOwnerComponent().getModel("cmodel");
		...
		_getPOHistoryData: function () {
			...
			if (proceed === "X") {
				...
				var lPromise = new Promise(function (resolve, reject) {
					...
					pohModel.read('/YY1_Open_GR_Details_API', {
						filters: oFilters,
							...
							if (oResponse.results.length > 0) {
								for (var j = 0; j < oResponse.results.length; j++) {
									...
									if (oResponse.results[j].GoodsMovementType === '101' &&
										...
										po.Supplier = oResponse.results[j].Supplier_01;
										po.SupplierName = oResponse.results[j].SupplierName_01;
										...
										po.Plant = oResponse.results[j].Plant_01;
									...
									if (oResponse.results[j].GoodsMovementType === '101' &&
										...
										gr.grquantityunit = oResponse.results[j].BaseUnit;
									...
									if (oResponse.results[j].GoodsMovementType === '102' &&
										...
										if (rgrindex === -1) {
											...
											that._rgrRec[rgrindex].rgrquantity = parseFloat(that._rgrRec[rgrindex].rgrquantity) + (parseFloat(rgr.rgrquantity) * -
												1);
									...
									if (oResponse.results[j].DebitCreditCode === 'S' &&
										...
										inv.invquantityunit = oResponse.results[j].BaseUnit;
								...
								var ndmsg = that.getView().getModel("i18n").getResourceBundle().getText("ndmsg");
								sap.ui.core.BusyIndicator.hide();
								sap.m.MessageBox.information(ndmsg);
							}
			...
			for (var j = 0; j < this._poRec.length; j++) {
				...
				for (var k = 0; k < this._grRec.length; k++) {
					...
					if (poout.PurchaseOrder === this._grRec[k].PurchaseOrder &&
						...
						poout.grquantityunit = this._grRec[k].grquantityunit;
				...
				for (var i = 0; i < this._invRec.length; i++) {
					if (poout.PurchaseOrder === this._invRec[i].PurchaseOrder &&
						...
						poout.invquantityunit = this._invRec[i].invquantityunit;
				...
				if (poout.qdiff !== 0) {
					...
					this._tamtdiff  = parseFloat(this._tamtdiff) + parseFloat(poout.damount);
				...
				this._tamtdiff   = parseFloat(this._tamtdiff).toFixed(2);
				this._total = "Total";
			}
		...
		_display: function () {
			...
			this._out.amtdiff = this._tamtdiff;
			this._out.Total = this._total;
			this._oModel.setData(this._out);
			otab.setModel(this._oModel);
			if ( this._out.rec.length === 0)
			{
				var nogrMsg = this.getView().getModel("i18n").getResourceBundle().getText("ngrm");
				sap.m.MessageBox.information(nogrMsg);
			}
		},
		searchSuppliers: function () {

			if (!this._sValueObj) {
				this._sValueObj = sap.ui.core.Fragment.load({
			...
			var sObj = sap.ui.getCore().byId("compinput");
			var dtokens = [];
			dtokens.push(dtoken);
			sObj.setTokens(dtokens);

			this._sValueObj.then(function (oTable) {

				that._helpDialog = oTable;
				var sfield = new sap.m.SearchField({
					sId: "sfield",
					showSearchButton: false
				});
				var fbobj = sap.ui.getCore().byId("sfbar");
				fbobj.setBasicSearch(sfield);
				// var fitemobj = new Fgitem({visibleInAdvancedArea:true,groupName:"CompanyFilter",groupTitle:"CompanyCode",name:"B",label:"Company Code"});
				// fitemobj.setControl(iobj);
				// var fbarobj = new Fbar({searchEnabled:true,showFilterConfiguration:false,advancedMode:true});

				// fbarobj.addFilterGroupItem(fitemobj);

				// oTable.setFilterBar(fbarobj);
				oTable.open();
				oTable.getTableAsync().then(function (oTab) {
					...
					oTab.setVisibleRowCountMode(sap.ui.table.VisibleRowCountMode.Auto);
					...
					var ofltr = [];
					var fobj = new Filter("CompanyCode", FilterOperator.EQ, "US01");
					ofltr.push(fobj);
					that._sarr = [];
					oTab.setBusy(true);
					that._sModel.read('/A_SupplierCompany', {
						filters: ofltr,
						urlParameters: {
							$expand: "to_Supplier"
						},
						success: function (oResponse) {
							for (var t = 0; t < oResponse.results.length; t++) {
								var tobj = {};
								tobj.Supplier = oResponse.results[t].to_Supplier.Supplier;
								tobj.SupplierName = oResponse.results[t].to_Supplier.SupplierName;
								that._sarr.push(tobj);

							}
							that._sObj.rec = that._sarr;
							that._nsModel = new sap.ui.model.json.JSONModel();
							that._nsModel.setData(that._sObj);
							oTab.setModel(that._nsModel);
							oTab.setBusy(false);
		...
		searchCompanyCodes: function () {
			if (!this._companyXMlObj) {
				this._companyXMlObj = new sap.ui.core.Fragment.load({
					name: "OpenGR.OpenGR.view.companyCodeHelp",
					controller: this
				});
			} else {
				this._cflag = "X";
			...
			var that = this;
			this._companyXMlObj.then(function (helpobj) {
				helpobj.setModel(that._coModel);
				that._chelpObj = helpobj;
				helpobj.open();
				helpobj.getTableAsync().then(function (oTab) {
					if (that._cflag !== "X") {
						var lblCuse = new Label({
							text: "{CompanyCode}"
						});
		...
		onSuppFilter: function () {
			var that = this;
			var ofltr = [];
			var sObj = sap.ui.getCore().byId("compinput");
			var tokens = sObj.getTokens();

			this._sTable.setBusy(true);
			for (var j = 0; j < tokens.length; j++) {
				var ccode = tokens[j].getKey();
				var fobj = new Filter("CompanyCode", FilterOperator.EQ, ccode);
				ofltr.push(fobj);
			}

			this._sarr = [];
			this._sTable.setBusy(true);
			this._sModel.read('/A_SupplierCompany', {
				filters: ofltr,
				urlParameters: {
					$expand: "to_Supplier"
				},
				success: function (oResponse) {
					...
					that._nsModel = new sap.ui.model.json.JSONModel();
					that._nsModel.setData(that._sObj);
					that._sTable.setModel(that._nsModel);
					that._sTable.setBusy(false);
		...
		onSuppSearch: function (data) {
			sap.m.MessageBox.information("handler is called");
		},
		...
		onCompanyHelpCancel: function () {
			this._chelpObj.close();

		},
		onCompanyHelpOk: function (oEvent) {
			var tokens = oEvent.getParameter("tokens");
			//	this._cTypes = tokens;
			var sObj = sap.ui.getCore().byId("compinput");
			sObj.setTokens(tokens);
		...
		onCompanyHelpAfterClose: function () {
			this._chelpObj.destroy();
		},
